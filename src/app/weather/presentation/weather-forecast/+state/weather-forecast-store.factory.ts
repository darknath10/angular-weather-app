...
export function weatherForecastStoreFactory(
  ...
  const setForecast = store.updater((state, forecast: WeatherForecast | null) => ({ ...state, forecast, loading: false }));
  const setLoading = store.updater((state, loading: boolean) => ({ ...state, loading }));

  const fetchWeatherForecast = store.effect((location$: Observable<Location>) => {
    return location$.pipe(
      tap(() => setLoading(true)),
      switchMap((location) => weatherForecastGw(location).pipe(
        tapResponse({
...
export type WeatherForecastStore = ReturnType<typeof weatherForecastStoreFactory>;
