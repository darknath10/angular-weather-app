...
const openMeteoHourlyForecastToCurrentWeatherByTimezone =
  (timezone: string) =>
  ({
    current_weather,
    hourly,
  }: OpenMeteoHourlyForecastResponse): CurrentWeather => {
...
export function openMeteoCurrentWeatherGatewayFactory(
  http: HttpClient,
  url: string
): CurrentWeatherGateway {
