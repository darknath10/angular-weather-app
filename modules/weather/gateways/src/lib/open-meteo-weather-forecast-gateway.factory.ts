...
export function openMeteoWeatherForecastGatewayFactory(
  http: HttpClient,
  url: string
): WeatherForecastGateway {
  return ({ latitude, longitude, timezone }: Location) => {
    const params = new HttpParams()
      ...
      .append(
        'daily',
        'weathercode,temperature_2m_max,temperature_2m_min,precipitation_sum,et0_fao_evapotranspiration'
      );

    return http.get<OpenMeteoForecastResponse>(url, { params }).pipe(
      map<OpenMeteoForecastResponse, WeatherForecast>(
        ({
          daily: {
            time,
            weathercode,
